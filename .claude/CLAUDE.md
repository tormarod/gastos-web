...
- **`templates/`** — Jinja2 HTML extending `base.html` (tabs Inicio · Movimientos · Revisar · Análisis · Subir, at the top on desktop and at the bottom on phones; Ajustes is a gear with the word, next to Salir; review badge): `home.html` (Inicio; categories link to their movements), `movements.html`, `analysis.html`, `category.html`, `settings.html` (Ajustes), `review.html`, `upload.html`, `login.html`. Chart.js loaded from CDN. No build step, no Node.
