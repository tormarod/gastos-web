...
- [ ] No `.pem` key is tracked: `git ls-files '*.pem'` should return nothing
...
- [ ] Every page except `login.html` extends `base.html` (navigation, review badge, bank alerts)
- [ ] Bank text inserted from JavaScript goes through `esc()` (no raw `innerHTML` with concepts)
